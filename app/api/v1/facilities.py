...
from app.models.models import AuditActionType, AuditOutcome, Facility, User
...
async def get_my_facility(
    ...
    if not facility:
        log_audit_event(
            ...
            action_type=AuditActionType.READ, resource_type="facility",
    ...
    log_audit_event(
        db, request=request, user=current_user,
        action_type=AuditActionType.READ, resource_type="facility",
        resource_id=facility.id, outcome=AuditOutcome.SUCCESS,
    )
    db.commit()
...
async def get_facility(
    ...
    if current_user.facility_id != facility_id:
        log_audit_event(
            db, request=request, user=current_user,
            action_type=AuditActionType.READ, resource_type="facility",
            resource_id=facility_id, outcome=AuditOutcome.FAILURE,
        )
        db.commit()
        raise HTTPException(
