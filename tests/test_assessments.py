...
def test_get_resident_assessments_facility_scoping(client, facility, db_session):
    ...
    response = client.get(f"/api/v1/assessments/resident/{other_resident.id}", headers=auth_headers(nurse))
    assert response.status_code == 404
...
def test_get_assessment_from_other_facility_returns_404(client, facility, db_session):
    ...
    db_session.add(other_resident)
    db_session.flush()
    other_assessment = _make_assessment(db_session, other_facility, other_resident, nurse)
    db_session.commit()

    response = client.get(f"/api/v1/assessments/{other_assessment.id}", headers=auth_headers(nurse))
    assert response.status_code == 404
